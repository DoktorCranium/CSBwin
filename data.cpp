...
{//Return true if file successfully.
...
#endif
  {
    for (i=0; i<MaxTraceFiles; i++)
    {
      ...
      if (TraceFile < 0)
      {
        /* 100 was too small and the compiler could prove it: the text below is
         * 28 characters plus a filename of up to 99, so 128 is the smallest
         * size that cannot truncate.  Sized rather than switched to snprintf
         * because this file is also compiled by HP C++ on OpenVMS. */
        char  msg[128];
