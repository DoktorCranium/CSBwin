# GLib IS NO LONGER A DEPENDENCY, so no target below asks pkg-config for it.
# stdafx.h includes csbglib.h in place of <glib.h> on every platform now (read
# its header comment); the variables are still called "gincludes"/"glibs"
# because every rule in this file uses those names, not because any GLib is
# involved.  What the engine used was fifteen typedefs, G_BYTE_ORDER, four
# integer<->pointer casts and six printf wrappers, all of which csbglib.h
# supplies - and OpenVMS, which has no GLib at all, has been built that way
# from the start.  See note 2 in CMakeLists.txt.
#
# Do not put `pkg-config --cflags glib-2.0` back: it makes GLib's headers
# visible again, and if anything ever includes <glib.h> alongside csbglib.h the
# build stops on purpose (modern GLib makes g_error/g_warning/g_critical macros
# that collide with csbglib.h's functions).
...
ifeq ($(target), Ubuntu_SDL20)
  ...
  gincludes =  `pkg-config --cflags sdl2`
...
stdafx.h: Objects.h  CSBTypes.h  csbglib.h
	touch stdafx.h
