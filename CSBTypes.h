...
/* The classic Mac OS 9 build, made with Metrowerks CodeWarrior from csb.mcp.
 *
 * THE TEST USED TO BE "#ifdef TARGET_OS_MAC" AND THAT IS NOW THE WRONG
 * QUESTION.  In 2004 TARGET_OS_MAC came from the CodeWarrior Mac headers and
 * meant this build; on Apple's current SDK <TargetConditionals.h> defines it as
 * 1 for EVERY Apple platform, and <SDL.h> includes that header.  So a modern
 * macOS build of the SDL/POSIX layer landed in here and died on the #error
 * below - clang is not CodeWarrior - and would have gone on to define
 * _bigEndian on a little-endian Mac, byte-swapping every field read from
 * DUNGEON.DAT.
 *
 * "macintosh" and "__MWERKS__" ask the question that was actually meant: both
 * are defined by the classic toolchain (macintosh explicitly, in Prefix.h) and
 * neither is defined by any Apple compiler since.  stdafx.h also undefines
 * TARGET_OS_MAC for the _LINUX layer - belt and braces, and its comment there
 * has the full story - but this arm no longer depends on that.
 *
 * The #error is kept rather than deleted: it says that a Toolbox build needs a
 * compiler that can reverse bitfield order, which is still true. */
...
/* Byte-align every structure declared from here on, and in every file that
 * includes this header - which is every file in the build, via stdafx.h.
 *
 * This is NOT an optimisation choice, it is the on-disk format: the structures
 * below and in CSB.h / Data.h are overlaid directly onto the bytes read from
 * DUNGEON.DAT, HCSB.DAT, HCSB.HCT, GRAPHICS.DAT and the saved games, all of
 * which are little-endian Atari-ST images with no inter-field padding.  Any
 * compiler-inserted padding silently reads garbage.
 *
 * On OpenVMS, use the native #pragma nomember_alignment rather than
 * #pragma pack(1).  HP C/C++ does accept #pragma pack, but nomember_alignment
 * is the documented OpenVMS spelling for exactly this ("align every member on
 * a byte boundary"), it cannot be confused with the Microsoft push/pop form,
 * and it is the one the rest of the OpenVMS world uses.  The two are
 * equivalent for our purposes; there is no reset in either case, which is
 * intentional - the packing is meant to apply for the remainder of the
 * translation unit.
 *
 * FOUR PLACES USE A pragma PAIR TO SAY SOMETHING DIFFERENT FOR ONE BLOCK, each
 * with a comment saying why.  WinScreen.cpp's Win32 DIB block re-states the
 * packing, because the SDL headers may have changed it by then.  The other three
 * go the other way and take NATURAL alignment: S12406 in Chaos.cpp, S20 and TEXT
 * in Hint.cpp.  Those three hold pointers and are statically initialised with
 * addresses, and Apple's arm64 linker refuses to relocate a pointer that does
 * not sit at a multiple of 8 ("ld: pointer not aligned").  They exist only in
 * memory - no file has their shape - which is what makes that legitimate.
 * Every structure that IS an image of file data stays packed. */
#ifdef __VMS
