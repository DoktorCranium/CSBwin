 ...
 * packing CSBTypes.h asked for.  This is the second of the two places in the
 * tree that restores alignment; the other is the DIB block in WinScreen.cpp,
 * whose comment explains the OpenVMS spelling of the pragmas.
 *
 * WHY IT HAS TO BE ALIGNED.  Byte-packed on a 64-bit target, the three
 * pointers land at offsets 2, 10 and 18, and the s12406[], fourbuttons[] and
 * s12406_4802 tables further down are statically initialised with the ADDRESSES
 * of dlgButton_12610, byte12618 and friends.  A static pointer initialiser is a
 * relocation, and Apple's arm64 linker refuses to write one at an offset that
 * is not a multiple of 8:
 *
 *     ld: pointer not aligned in '_s12406'+0x6A (Chaos.cpp.o)
 *
 * 0x6A is 4*26 + 2 - the pDlgButton2 of s12406[4].  ELF linkers accept the
 * same object, which is why neither Linux nor the OpenVMS build ever saw this.
 *
 * WHY IT IS SAFE.  S12406 exists only in memory.  It is the ST image's dialog
 * descriptor lifted out into C globals - see "//i8 Byte12406;(use s12406)" in
 * the d overlay above, which is the member it replaced - so no file, save game
 * or recording contains one.  Its layout has in any case already left the ST's
 * behind on any 64-bit build: the field names say 2, 6 and 10 because ST
 * pointers were four bytes, and pnt6 has not been at offset 6 since the engine
 * was first compiled for a 64-bit machine.  Every access is by field name or by
 * array index (checked: no sizeof(S12406), no casts to it, no byte arithmetic
 * on one), so the only thing this changes is 26 bytes per entry becoming 32. */
#ifdef __VMS
...
// *********************************************************
//          TAG000588
RESTARTABLE _UtilityDialogBox(const char *P1, i32 p2, const char *p3, S12406 *P4) //VMS: no top-level const - see COMPILER_FIXES 12.10
{ //(i16)
  // Generic Diaog Box??
  ...
  P3 = p3;
//;;;;;;;;;;;;;;;;;;;;
  /* 24 was six ST pointers of four bytes each, and this array has six entries.
   * On a 64-bit build it is 48 bytes, so the literal cleared slots 0 to 2 and
   * left 3 to 5 holding whatever the PREVIOUS call to this function put there -
   * the array is static.  That matters: it becomes the active dialog through
   * SetActiveDialog() below, and TAG002faa() and the two loops around line 3700
   * walk all six entries looking for NULL.  Correct on VMS Alpha and on the ST
   * (32-bit pointers), wrong on every 64-bit build including macOS, so it is
   * sizeof() now.  Found while checking what the alignment change above could
   * touch; it is not caused by it. */
...
//
// *********************************************************
RESTARTABLE _TAG000ede(i32 P1, char *P2) //VMS: no top-level const - see COMPILER_FIXES 12.10
{//(i16)
  static dReg D0;
  static aReg A0;
  static i16 w_2;
...
//
// *********************************************************
RESTARTABLE _TAG000fc4(i32 P1) //VMS: no top-level const - see COMPILER_FIXES 12.10
{//(void)
...
//
// *********************************************************
RESTARTABLE _TAG00179c(i32 P1) //VMS: no top-level const - see COMPILER_FIXES 12.10
{//(i16)
...
// *********************************************************
//  TAG001868
RESTARTABLE _ReadSavedGame(i32 P1) //VMS: no top-level const - see COMPILER_FIXES 12.10
{//(i16)
...
//;;;;;;;;;;;;;;;;;;
  ...
  TextToScreen(D0W,
               ...
               9,
               1,
               //Underline of D1W characters: the tail of a 19-character run of
               //underscores.  Subscripting says that plainly; the equivalent
               //"literal + 19 - D1W" reads to clang as a suspected attempt to
               //append to a string (-Wstring-plus-int).
