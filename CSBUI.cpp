...
// VMS: SDL_strupr is declared by SDL_stdinc.h but is NOT exported by the SDL 1.2
// shareable image (LIBSDL$SHR), so a reference to it links as an undefined
// symbol and calls address 0.  Use the same local upcaser as the Mac path.
//
// CSB_NO_SDL_STRUPR is the same escape hatch for a Unix SDL 1.2 that does not
// export it either.  The declaration is unconditional in SDL_stdinc.h, so the
// only way to find out is to link a one-line program against the library the
// build will actually use, which is what ./configure does; the failure it
// prevents is a link-time undefined symbol, not a compile error, so nothing in
// the sources can detect it.  A local six-line upcaser is no loss: the two
// calls here are on dungeon and save-game names, ASCII by construction.
#if defined(TARGET_OS_MAC) || defined(__VMS) || defined(CSB_NO_SDL_STRUPR) //007
...
#endif //009
//   Built-in movement keys, and the automap keys.
//
//   Upstream ships no default key bindings at all: keyxlate is filled only
//   from CONFIG_NAME, so with no config file every key is dead and the party
//   can be moved only by clicking the arrows.  These six letters are always
//   bound:
//
//       Q W E      Q turn left       W move forward     E turn right
//       A S D      A sidestep left   S move backward    D sidestep right
//
//   and so is one more, for the automap at the end of Viewport.cpp:
//
//       M          show the map, and press it again to put the 3D view back
//
//   Each one is translated to itself, which puts it in the keyQueue for the
//   main loop to hand to TAG0196da (CSBCode.cpp) - the engine's own
//   keyboard-to-button translator, which is where the letters are turned
//   into movement.  Both cases are bound because SDL reports the unshifted
//   keysym while the Win32 layer reports the shifted character.
//
//   Mode 1 is 'adventuring', so these never interfere with typing a
//   champion's name (mode 2).
static void AddBuiltinMovementKeys(void)
{
  static const char keys[] = "qweasdmQWEASDM";
  for (i32 i=0; keys[i] != 0; i++)
  {
    keyxlate.addkey(keys[i], keys[i], 1, TYPEKEY);
  };
}
...
extern ui32 TImER;
    ...
    if (numerror==0)
    {
      ...
      //The %.100s says the same thing as the line above to the compiler, which
      //cannot see that truncation and otherwise warns that buf's full 501
      //bytes might not fit in msg's 200 (-Wformat-overflow at -O).  Behaviour
      //is unchanged: buf is already at most 100 characters by here.
  ...
  CLOSE(f);
  //Last, so that a KEY line in the config file - which translate() finds
  //first - still overrides the built-in binding.
  AddBuiltinMovementKeys();
}
...
#endif //017
#ifdef __VMS //017a
  /* DELIBERATE DEVIATION FROM THE LINUX BUILD.
   *
   * On Linux this function is a no-op: the usleep() call above is behind
   * "#ifdef xxxx", i.e. switched off, and there is no _LINUX branch.  The only
   * caller is CSBCode.cpp's main animation loop -
   *
   *     if (VBLMultiplier == 1) UI_Sleep(20);
   *
   * - which is the frame pacer.  With UI_Sleep doing nothing, that loop spins
   * as fast as the CPU allows and the process burns 100% of a core for the
   * whole session; on Linux it merely looks like a busy game.
   *
   * OpenVMS systems are routinely shared, and a batch/interactive process
   * pinning a CPU for hours is not acceptable behaviour there - it will hit
   * CPULM/CPU time limits, distort accounting and starve other users.  So the
   * OpenVMS build implements the sleep, with SDL_Delay rather than a
   * VMS-specific $HIBER/lib$wait: SDL 1.2 is already linked in, SDL_Delay's
   * granularity is what the rest of the SDL timer code assumes, and it matches
   * the semantics of the Windows branch above exactly (Sleep(milliseconds)).
   *
   * NoSleep is honoured for the same reason the Windows branch honours it:
   * Recording.cpp sets it from a "#NoSleep" directive in a recorded game so
   * that playback runs at full speed instead of in real time. */
  if (!NoSleep && (milliseconds > 0)) SDL_Delay((Uint32)milliseconds);
#endif //017a
}
...
FILE *UI_fopen(const char *name, const char *flags)
{
#ifdef __VMS //034a
  /* -------------------------------------------------------------------------
   * OPENVMS: BINARY FILES MUST BE OPENED AS A BYTE STREAM, EXPLICITLY.
   *
   * This is the single most important line of the OpenVMS port, and the one
   * whose absence does not produce an error message.
   *
   * On OpenVMS a file is not a byte stream by default - it is a sequence of
   * RMS RECORDS.  The "b" in fopen(name,"rb") does nothing here: there is no
   * text/binary newline translation to switch off, so the C RTL ignores it and
   * still goes through record-oriented processing.  What that means in
   * practice:
   *
   *   WRITING.  A file created by fopen() gets the default format, which is
   *   variable-length records with implied carriage control (rfm=var,rat=cr).
   *   fwrite() of arbitrary binary data then breaks the stream into records -
   *   and because rat=cr says "there is a newline at the end of every record",
   *   reading it back returns the data with an 0x0A INSERTED at every record
   *   boundary that the writer never wrote.  A saved game written that way is
   *   corrupt, and nothing reports it: the corruption surfaces later as a
   *   nonsense dungeon, a die() with a hex code, or an ACCVIO.
   *
   *   READING.  A data file transferred to OpenVMS in image mode usually lands
   *   as fixed-length 512-byte records; read record-wise, the last record is
   *   padded out to 512 bytes, so the file appears longer than it is.
   *
   * The only way to get a true byte stream is the variadic RMS option
   * "ctx=stm", which cannot be expressed in the mode string - hence this
   * wrapper.  The sibling ports in this tree hit exactly this: see section 3 of
   * [-.ROTT]COMPILER_FIXES.TXT, which had to add four VMS_OPEN_* macros because
   * ROTT calls open() in four places.  CSBwin is much easier - EVERY file the
   * game opens comes through this one function (system.cpp OPEN(), CREATE(),
   * FILENAME::Open() in VMSFileName.inl, and Sound.cpp's socko.wav) - so one
   * fix covers the whole engine.
   *
   * WHY THE 'b' TEST IS THE RIGHT DISCRIMINATOR.  The engine is consistent:
   * binary data uses "rb"/"wb" (DUNGEON.DAT, GRAPHICS.DAT, HCSB.DAT, saved
   * games, DUNGEON.FTL, socko.wav), while the genuinely line-oriented files use
   * "r"/"w" (the traces, the recorded-session logs, annotation.txt,
   * Translation.txt, HINTDUMP.TXT).  Those SHOULD stay as records: that is what
   * makes them TYPE-able, EDIT-able and searchable by every DCL tool, which is
   * the whole point of a text file on this operating system.  So text files are
   * deliberately left to the default path below.
   *
   * WHY THE ATTRIBUTES ARE SPLIT.  "rfm" and "rat" describe how a file is
   * CREATED and are ignored when an existing file is opened; "ctx=stm"
   * describes how THIS PROGRAM accesses it and applies either way.  Passing
   * only what is meaningful keeps a read-only open from looking as though it
   * intends to change the file's attributes.
   * ------------------------------------------------------------------------- */
  if (strchr(flags, 'b') != NULL)
  {
    if ((strchr(flags, 'w') != NULL) ||
        ...
        (strchr(flags, '+') != NULL))
    {
      /* May create the file: say what it should look like when it does.
       * stream_LF with no carriage-control attribute is the standard
       * "just the bytes" format on OpenVMS, and it is what every other
       * platform's copy of these files already is. */
      return fopen(name, flags, "ctx=stm", "rfm=stmlf", "rat=none");
    };
    /* Read-only: cannot create, so ask only for stream access. */
    return fopen(name, flags, "ctx=stm");
  };
#endif //034a
