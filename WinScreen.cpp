...
#ifdef _LINUX
/* The three structures below are the Win32 DIB descriptors, hand-declared
 * because there is no <windows.h> here.  They are byte-packed and then natural
 * alignment is RESTORED at the end of the block (the #pragma pack() /
 * #pragma member_alignment restore below).  Everywhere else the packing set in
 * CSBTypes.h is meant to run to the end of the translation unit; the only other
 * places that step out of it are the S12406 declaration in Chaos.cpp and S20 and
 * TEXT in Hint.cpp, which do the opposite of this block - they take natural
 * alignment for one structure and then go back to byte packing, because their
 * static tables hold pointers and Apple's arm64 linker will not relocate a
 * pointer at an unaligned offset.  Those three comments have the story.
 *
 * On OpenVMS this uses the native save/restore pair.  #pragma pack() with empty
 * parentheses is the Microsoft "reset to default" form and is not the
 * documented OpenVMS spelling, whereas
 *     #pragma member_alignment save / #pragma nomember_alignment / restore
 * is, and restore puts back exactly the state CSBTypes.h left (byte packing)
 * rather than resetting to the compiler default.  That difference is harmless
 * here: nothing after this block in WinScreen.cpp declares a structure - the
 * on-disk types it uses all come from CSB.h and Data.h, which were included
 * above - but "restore what was there" is the safer of the two behaviours if
 * that ever stops being true.
 *
 * Packing is in fact irrelevant to correctness for these three: BITMAPINFOHEADER
 * is 4- and 2-byte fields in an order that is already naturally aligned, so it
 * is 40 bytes either way, which is what matters because DumpWindowToBMP()
 * fwrite()s it as a .BMP header.  The pragmas are kept because that is a
 * property of the current field list, not a guarantee. */
#ifdef __VMS
