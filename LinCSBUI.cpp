...
static bool LIN_PlaySound(i8* audio, const ui32 /*size*/, int volume)
{
  ...
  /* "The sound was started" - and the caller does look: SOUNDER::Play() in
   * Sound.cpp calls UI_StopSound() and requeues the wave when this comes back
   * false, and SOUNDER::CheckQueue() keeps the wave buffer alive on a false.
   * Falling off the end here (the -Wreturn-type warning in this file) handed
   * both of them whatever happened to be in the return register.  TRUE is what
   * every path of the pre-SDL implementation in LinCSBUI_orig.cpp returned. */
  return true;
};
...
#ifdef __VMS //017c
    /* ---------------------------------------------------------------------
     * DCL UPPERCASES A FOREIGN COMMAND'S ARGUMENTS.
     *
     * CSBWIN.EXE has to be run as a foreign command
     *     $ CSBWIN == "$DKA0:[CSB]CSBWIN.EXE"
     * because that is the only way DCL hands the rest of the line to _argv
     * instead of trying to read it as DCL qualifiers.  But DCL UPCASES a
     * foreign command's arguments unless they were quoted, so
     *     $ CSBWIN --directory DKA0:[CSB]
     * arrives here as "--DIRECTORY", every strcmp below fails, and the
     * option is SILENTLY IGNORED - there is no else clause that complains.
     * The player then sees "Unable to open DUNGEON.DAT" from a command line
     * that looks exactly right.  That is the worst possible failure mode, so
     * fold the switch name to lower case before comparing it.
     *
     * DOING THIS IN STATE 0 ONLY IS THE WHOLE POINT.  State 0 never keeps the
     * argument, it only compares it.  The arms that DO keep one -
     * enum_directory -> folderParentName, enum_module -> folderName,
     * enum_path -> root, enum_dungeon -> dungeonName, enum_play ->
     * PlayfileName - run with state != 0 and are untouched here.  They store
     * the argv pointer ITSELF, so handing them this stack buffer would leave
     * a dangling pointer, and case-folding a path would be wrong anyway: an
     * ODS-5 volume can hold lower- and mixed-case file names.
     *
     * Quoting every switch ($ CSBWIN "--directory" "DKA0:[CSB]") also works,
     * and so does
     *     $ DEFINE DECC$ARGV_PARSE_STYLE ENABLE
     *     $ SET PROCESS/PARSE_STYLE=EXTENDED
     * Both are in README.VMS, but neither can be relied on being set, and
     * the truncation below cannot cause a false match: every switch name is
     * under 20 characters, so a 63-character truncation never equals one.
     * --------------------------------------------------------------------- */
	char vmsKey[64];
	if (state == 0)
	{
	  int i;
	  for (i = 0; (i < (int)sizeof(vmsKey) - 1) && (key[i] != 0); i++)
	    vmsKey[i] = (char)tolower((unsigned char)key[i]);
	  vmsKey[i] = 0;
	  key = vmsKey;
	};
#endif //017c
...
*/
  /* Lock FIRST, then read ->pixels.  It was the other way round, which is
   * defined behaviour only for the surfaces SDL keeps in ordinary memory: for
   * anything the driver has to lock (and on macOS the video surface is the
   * back end of a Cocoa window, not a buffer this process owns), ->pixels is
   * documented as meaningless until the lock succeeds and may move across one.
   * The two lines cost nothing in the order that is always correct. */
...
/* THE SDL12 CONFIGURATION DID NOT LINK WITHOUT THESE.
 *
 * WinScreen.cpp calls all three unconditionally around each frame
 * (WinScreen.cpp:1543 UI_ScreenStartUpdates, :1674 UI_ScreenEndUpdates, :1678
 * UI_ScreenPresent) and UI.h:150-152 declares them for every platform - but
 * the only definitions in the tree were the SDL20 arm below and the
 * _MSVC_INTEL arm in CSBUI.cpp:1932-1934.  Nothing defined them for SDL12, so
 * "-D_LINUX -D SDL12" produced 41 clean objects and three undefined symbols at
 * link time.  (Found by linking the OpenVMS build set; it is not a
 * VMS-specific problem and the fix is deliberately not #ifdef __VMS, since
 * gating it that way would leave the Linux SDL12 build unlinkable.)
 *
 * Empty bodies are the correct implementation here, exactly as they are on
 * Windows.  The three hooks exist for SDL 2's lock-texture / unlock-texture /
 * present cycle, which has to bracket a whole frame.  SDL 1.2 has no such
 * cycle: UI_SetDIBitsToDevice above is self-contained - it locks the video
 * surface, blits, unlocks and calls SDL_UpdateRect for its own rectangle - so
 * there is nothing left for a frame-level bracket to do. */
