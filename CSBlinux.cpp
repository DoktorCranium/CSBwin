...
//           DumpEvents();
  ...
  {
    /* An unrecognised timer code: ask the main loop to quit.  The explicit
     * "return 0" is not decoration - this is an SDL_AddTimer callback, and
     * falling off the end of it returned an unspecified interval to SDL
     * (undefined behaviour, and the one -Wreturn-type warning in this file).
     * 0 is SDL 1.2's "do not reschedule me", which is what a quit wants. */
...
#if defined SDL12
/* ---------------------------------------------------------------------------
 * SetDisplayMode - THE only place in the SDL 1.2 build that calls
 * SDL_SetVideoMode, so that WND, fullscreenActive and "is there a surface at
 * all?" cannot get out of step.  Returns false only when there is no surface.
 *
 * There were three copies of the call - start-up, Alt+Enter, and the
 * VIDEORESIZE handler - and only the first checked the result.  That is not a
 * style point: SDL 1.2 gives up the old surface before it tries the new mode,
 * so a REFUSED mode leaves WND and SDL_GetVideoSurface() both NULL, and the
 * next frame goes straight into SDL_GetVideoSurface()->pixels in
 * LinCSBUI.cpp's UI_SetDIBitsToDevice without a check.  So Alt+Enter into a
 * mode the display cannot do was an immediate crash, not a refusal.
 *
 * That is a real case on macOS rather than a hypothetical one.  SDL 1.2's
 * Quartz backend implements fullscreen by switching the whole DISPLAY to the
 * requested size and depth, and no Mac made this century has a 640x400 16-bit
 * mode; on Apple silicon the attempt commonly fails outright.  Hence the
 * fallback: ask for what was wanted, and if that is refused ask for the other
 * kind of mode rather than carrying on with nothing to draw on.  A window
 * that is not what you asked for is still a game you can play.
 *
 * fullscreenActive comes from WND->flags - what SDL actually GAVE us - and not
 * from what was requested, because after a fallback those differ and every
 * later Alt+Enter depends on this flag being the truth.
 * ------------------------------------------------------------------------- */
static bool SetDisplayMode(bool wantFullscreen)
{
  SDL_Surface *surface;
  surface = SDL_SetVideoMode(WindowWidth, WindowHeight, 16,
                             SDL_SWSURFACE | (wantFullscreen ? SDL_FULLSCREEN : 0));
  if (surface == NULL)
  {
    printf("%s %d x %d refused (%s) - trying the other mode.\n",
           wantFullscreen ? "Fullscreen" : "Window",
           WindowWidth, WindowHeight, SDL_GetError());
    surface = SDL_SetVideoMode(WindowWidth, WindowHeight, 16,
                               SDL_SWSURFACE | (wantFullscreen ? 0 : SDL_FULLSCREEN));
  };
  WND = surface;
  fullscreenActive = (surface != NULL) && ((surface->flags & SDL_FULLSCREEN) != 0);
  if (surface == NULL) return false;
  printf("Display is %d x %d %s\n", surface->w, surface->h,
         fullscreenActive ? "fullscreen" : "in a window");
  return true;
}
#endif //SDL12
...
//  static int prevp1 = -1;
  MTRACE("SDL_KEYDOWN\n");
#if defined SDL12
  /* Leftover keystroke tracing to a hard-coded tmpfs path.  "/run/shm" is a
   * Linux-only filesystem, so on OpenVMS (and on any Linux without it) the
   * fopen fails and the unchecked fprintf dereferences a NULL FILE*, killing
   * the game on the first key press.  Behaviour where the path DOES exist is
   * unchanged. */
  {
    FILE *f;
    f = fopen("/run/shm/debug", "a");
  ...
  if ((evert.key.keysym.mod & (KMOD_RALT | KMOD_LALT)) != 0)
  {
    ...
    {
      /* Both arms were the same code with one flag different, and neither
       * checked the result - see SetDisplayMode above for why that mattered.
       * The two guards stay: refusing to leave fullscreen for a window that
       * would not fit on the desktop is the right answer, and it has to be
       * decided BEFORE the mode is dropped. */
      if (fullscreenActive)
      {
        if (WindowWidth > screenWidth) return;
        if (WindowHeight > screenHeight) return;
      };
      if (!SetDisplayMode(!fullscreenActive))
      {
        UI_MessageBox(SDL_GetError(), "Failed to change video mode", MESSAGE_OK);
        die(0x731d);
      };
      csbMessage.type=UIM_REDRAW_ENTIRE_SCREEN;
      if (CSBUI(&csbMessage) != UI_STATUS_NORMAL)
...
#if defined SDL12
  /* This whole function is unreachable in the SDL 1.2 build - nothing pushes
   * SDL_VIDEORESIZE (__resize_screen's body is commented out) and the window is
   * not SDL_RESIZABLE - but it is left correct rather than left as a trap for
   * whoever makes it reachable.  Two things were wrong with what was here:
   *
   *   SDL_FreeSurface(WND)  the video surface belongs to SDL, which says never
   *                         to free it, and SDL_SetVideoMode replaces it anyway;
   *   SDL_FULLSCREEN        a resize is not a request to change window/full
   *                         screen, so keep whichever we are in. */
  if (!SetDisplayMode(fullscreenActive))
  {
    UI_MessageBox(SDL_GetError(), "Failed to resize window", MESSAGE_OK);
    die(0x731e);
  };
#elif defined SDL20
...
// ****************************** the DISPLAY ****************************************************
  {
    ...
    /* current_w/current_h are the DESKTOP size, and the video driver fills them
     * in only if it knows it.  SDL's own "dummy" driver reports 0 x 0, and so do
     * some remote X servers.  Zero does not mean "a screen with no pixels", it
     * means "not known" - but the test below reads it as the first, and then
     * every window is too large for the screen and the game exits before it has
     * drawn anything.  ("SDL_VIDEODRIVER=dummy ./CsbWin" did exactly that, which
     * is how this was found.)  Treat unknown as "the window fits". */
    if ((screenWidth <= 0) || (screenHeight <= 0))
    {
      printf("Video driver does not report a desktop size"
             " - assuming %d x %d is usable.\n", WindowWidth, WindowHeight);
    ...
    /* Honour the request rather than forcing fullscreen.
     *
     * The Linux build always adds SDL_FULLSCREEN here and sets
     * fullscreenActive unconditionally, ignoring fullscreenRequested (which
     * config.txt's SIZE=FULL and the --fullscreen option set).  Under
     * DECwindows/CDE that is the wrong default for a port: a fullscreen mode
     * switch on an X server driven over a network, or on a display whose
     * modes the server will not change, gives you a black screen with no
     * visible way back - and the only way back IS visible only if you already
     * know it (Alt+Enter, handled in Process_SDL_KEYDOWN).  So start windowed
     * unless fullscreen was actually asked for; Alt+Enter still toggles.
     *
     * macOS IS THE SAME ARGUMENT, ONLY WORSE, which is why __APPLE__ is here
     * too.  SDL 1.2's Quartz backend takes fullscreen literally: it captures
     * the display and switches it to 640x400 at 16 bits.  Macs have not had
     * such a mode since CRTs, so the request is either refused - and until
     * SetDisplayMode was written that was a crash rather than a message - or
     * granted as a scaled emulation of it, which is a mode switch of the whole
     * desktop for a 320x200 game.  Neither is what someone starting the game
     * from a terminal window expects.  A Mac now starts windowed and
     * "--fullscreen" still works if that is what is wanted.
     *
     * TRU64 UNIX (__osf__) IS THE DECWINDOWS ARGUMENT AGAIN, on the same
     * hardware.  It is the first case above with the VMS taken off: an Alpha
     * running CDE, often a fixed-mode framebuffer or an X terminal, and an X
     * server of a generation before XFree86's mode-switching extension, which
     * is how SDL 1.2 changes modes at all.  What that produces is either a
     * screen-sized window with a 320x200 game somewhere in it or nothing
     * visible, and in both cases the way out is a keystroke you have to know
     * already.  A machine that is being used over a network to begin with is
     * the last one to take the desktop away from.
     *
     * The Linux and Raspberry Pi defaults are deliberately NOT changed: that
     * build has always come up fullscreen, people have muscle memory for it,
     * and nothing about a current X11 desktop makes it a trap the way the
     * three cases above are. */
    {
      bool wantFullscreen;
#if defined(__VMS) || defined(__APPLE__) || defined(__osf__)
      wantFullscreen = fullscreenRequested;
#else
      wantFullscreen = true;
#endif
      if (!SetDisplayMode(wantFullscreen))
      {
...
#else
#ifdef SDL12
    /* SCRAP is the 8-bit staging surface LinScreen.cpp draws into before
     * blitting to WND.  This build does not compile LinScreen.cpp - it uses
     * WinScreen.cpp, which renders into the engine's own 16-bit bitmap and
     * hands that to UI_SetDIBitsToDevice - so SCRAP is allocated and then
     * never touched again except by the (unreachable, the window is not
     * SDL_RESIZABLE) VIDEORESIZE handler, which recreates it at
     * 320*screenSize x 200*screenSize.
     *
     * Sizing it to the whole DESKTOP is therefore both inconsistent with that
     * handler and a pure waste: on a 1920x1080 display it is 2MB of process
     * working set that is never read.  On OpenVMS that matters more than it
     * does on Linux - a user's PGFLQUO/WSEXTENT is a fixed quota, not an
     * elastic one - so allocate the window size instead.  Kept as-is
     * elsewhere: the Linux build is what it is. */
#ifdef __VMS
    SCRAP = SDL_CreateRGBSurface(SDL_SWSURFACE,WindowWidth,WindowHeight,8,0,0,0,0);
#else
  ...
  {
#if defined SDL12
    /* "fullscreenActive = true;" was here and is now wrong to do: SetDisplayMode
     * has already set it from WND->flags, which is what SDL actually gave us.
     * Those two differ whenever the fullscreen mode was refused and the window
     * fallback was taken - and then this line made Alt+Enter's first press try
     * to LEAVE a fullscreen mode the game is not in, so the display stayed a
     * window and the flag flipped to false, i.e. the key appeared dead once. */
