...
//   The automap, at the end of Viewport.cpp.  Declared here rather than in
//   CSB.h because every object in this tree depends on every header, so a
//   line in CSB.h costs a full 41-file rebuild for three call sites.
...
tag0006d0:
  ...
  //   Remember where the party has been, for the automap at the end of
  //   Viewport.cpp.  Doing it here rather than in MoveParty is what makes
  //   teleports, falls down a pit, stairs and being thrown by a teleporter
  //   count as well as walking; none of those go through MoveParty.  It
  //   touches nothing but its own array, so it cannot alter a recording.
  AutomapMarkVisited();
  if (d.SelectedCharacterOrdinal != 0) goto tag000730;
  if (d.PartySleeping != 0) goto tag000730;
  if (automapActive)
  {
    //   The map has taken the viewport over, exactly as the inventory
    //   screen does; a second press of M clears the flag and DrawViewport
    //   takes the viewport back on the tick after that.
    AutomapDraw();
    goto tag000730;
  };
...
// *********************************************************
//   TAG001aa8
void PrintLines(i32 color, const char *Text) //VMS: no top-level const - see COMPILER_FIXES 12.10
{//(void)
  i32 lastPrintCol;
  dReg D0, D6;
  i8  LOCAL_56[54];
...
// *********************************************************
//   TAG01992e
MOVEBUTN *MoveParty(i32 button) //VMS: no top-level const - see COMPILER_FIXES 12.10
{//(void)
  // 01 Turn Left
  // 02 Turn Right
  // 03 Move forward
...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  if (pxA3 != NULL)
  {
    ...
    if (D6W != d.MouseQStart)
    {
      ...
      //   The built-in movement keys, bound by AddBuiltinMovementKeys in
      //   CSBUI.cpp:
      //
      //       Q W E      Q turn left       W move forward     E turn right
      //       A S D      A sidestep left   S move backward    D sidestep right
      //
      //   The KeyXlate tables searched above hold the original Atari scan
      //   codes, which are all of the form 0xSS00, so a plain letter can
      //   never collide with one.  The numbers below are the on-screen button
      //   numbers listed at the top of MoveParty.  Doing this here rather
      //   than in the UI layer means the keys are ignored whenever the engine
      //   has cleared pKeyXlate2 - while a menu is up, for instance - and
      //   that they queue, throttle, flash the arrow and record exactly as a
      //   click on the arrow does.
      D5W = 0;
      switch (key & 0xffff)
      {
      case 'q': case 'Q': D5W = 1; break; //Turn left
      case 'w': case 'W': D5W = 3; break; //Move forward
      case 'e': case 'E': D5W = 2; break; //Turn right
      case 'a': case 'A': D5W = 6; break; //Sidestep left
      case 's': case 'S': D5W = 5; break; //Move backward
      case 'd': case 'D': D5W = 4; break; //Sidestep right
      //   The automap, at the end of Viewport.cpp.  0x2000 is not a real
      //   button; it is one of the unused numbers in the list of special
      //   values at the end of struct MouseQueueEnt (CSB.h), and
      //   _HandleMouseEvents (Mouse.cpp) turns it into the call that shows
      //   or hides the map.  It goes through the mouse queue like
      //   everything else so that a recorded session replays with the map
      //   going up and down in the same clock ticks.
      //
      //   One key, doing both jobs.  ESC used to be the way back and did
      //   not work: the keypress never reached here, and nothing in the
      //   path between SDL and this switch treats 0x1b differently from a
      //   letter, so the most likely culprit is the window manager or the
      //   X server taking Escape for itself.  A toggle needs no second key
      //   and cannot be intercepted while the first one still works.
      case 'm': case 'M': D5W = 0x2000; break; //Show the map / put it away
      };
      if (D5W != 0)
      {
        D0W = D6W;
        ...
        pMouseQueue[D6W].num = D5W;
#endif
        pMouseQueue[D6W].x = 0; //A move ignores them, but they get recorded
        pMouseQueue[D6W].y = 0;
        d.MouseQEnd = D6W;
      };
    };
...
//
  ...
  //This was "D6L =+ ..." - a "+=" with the characters swapped, which C reads as
  //an assignment of a unary plus, so the three sizes accumulated above were
  //thrown away and FreeTempMemory() below was told to release only the last
  //allocation.  The two lines above it say what was meant.  (clang: "use of
...
// *********************************************************
//   TAG0204bc
RESTARTABLE _SelectSaveGame(i32 P1, i32 checkExist, i32 alwaysDate) //VMS: no top-level const - see COMPILER_FIXES 12.10
{//(void)
