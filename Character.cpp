...
//;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  else
  {
    ...
    {
      /* "#ifdef", not "#if": this is the only site in the tree that tested
       * TARGET_OS_MAC by value, and every other one asks whether it is defined.
       * stdafx.h undefines it for the SDL/POSIX layer (its comment says why),
       * and Apple's clang has -Wundef-prefix=TARGET_OS_ on by default, so an
       * "#if" on the undefined name is a warning on macOS.  The two spellings
       * mean the same thing here: the CodeWarrior Mac OS 9 build defines it as
       * 1, and nothing else defines it at all. */
#ifdef TARGET_OS_MAC
      Flood(A3, 8, 16);
#else
      Flood((ui8 *)A3, 4, 16);
